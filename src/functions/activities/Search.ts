    ...
    private async bingSearch(searchPage: Page, query: string) {
        ...
        for (let i = 0; i < 5; i++) {
            try {
                ...
                const scrollPromise = searchPage.evaluate(() => {
                    window.scrollTo(0, 0);
                });
                const timeoutPromise = new Promise(resolve => {
                    setTimeout(resolve, timeout, "timeout");
                });
                const result = await Promise.race([scrollPromise, timeoutPromise]);
                if (result === "timeout") {
                    this.bot.log(this.bot.isMobile, 'SEARCH-BING', 'Scroll timeout → Reloading page...', 'warn')
                    await searchPage.reload({ timeout: 15000 });
                }
